...
public class Client_main {
	...
	private void initialize() {
		...
		table = new JTable(new DefaultTableModel(new Object[] {"No.","Name","Barcode No.","Price","Quantity"},0));
			...
			public void actionPerformed(ActionEvent e){
		        new Thread(() -> {
					...
					try {
						//Add data to table
						cc = future.get();
			            integer=Integer.parseInt(cc);
						String get="";
						try {
							if(shop==1) {
							get = ClientGUI.getData1(integer);
							}
							else if(shop==2) {
							get = ClientGUI.getData2(integer);
							}
							else {
							get = ClientGUI.getData3(integer);
							}
							String[] getpart=get.split(" ");
							String name="";
							String[] getname=getpart[0].split("\\:");
							name=getname[1];
							if(checkname(name,model,table)) {
								String[] getprice=getpart[1].split("\\:");
								String price=getprice[1];
								int z=quantity();
								if(z!=0) {
						    	model.addRow(new Object[] {row,name,integer,price,z});
					            row=row+1;
								}
		...
		btnNewButton_1.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
			    ...
			    for (int i = rowCount - 1; i >= 0; i--) {
			    	int valuex=(int)table.getValueAt(i, 2);
			    	int valuey=(int)table.getValueAt(i, 4);
			    	String stringvaluey=Integer.toString(valuey);
			    	String namex=(String)table.getValueAt(i,1);
			    	try {
			    		//wait for shop
			    		if(shop==1) {
			    			ClientGUI.update1(valuex, valuey);
			    			ClientGUI.History("shop1", LocalDateTime.now().toString(), namex, stringvaluey);
			    			model.removeRow(i);
			    		}
			    		if(shop==2) {
							ClientGUI.update2(valuex, valuey);
							ClientGUI.History("shop1", LocalDateTime.now().toString(), namex, stringvaluey);
					        model.removeRow(i);
				    	}
			    		if(shop==3) {
							ClientGUI.update3(valuex, valuey);
							ClientGUI.History("shop3", LocalDateTime.now().toString(), namex, stringvaluey);
					        model.removeRow(i);
				    	}
